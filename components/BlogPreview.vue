    ...
    <div class="max-w-6xl mx-auto">
      <div class="text-center mb-16">
        ...
        <p class="text-xl text-gray-400 max-w-2xl mx-auto">
          Insights on web development, software engineering, and the latest in tech
        </p>
      </div>

      <!-- Blog posts preview -->
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12" v-if="posts?.length">
        <article 
        ...
        >
          <NuxtLink :to="post._path" class="block h-full">
            ...
            <!-- Title -->
            <h3 class="text-xl font-semibold mb-3 group-hover:text-blue-400 transition-colors line-clamp-2">
              {{ post.title }}
            </h3>
            ...
            <div class="flex flex-wrap gap-2 mb-4" v-if="post.tags">
              ...
              <span v-if="post.tags.length > 2" class="px-2 py-1 bg-gray-500/20 text-gray-400 text-xs rounded-full">
                +{{ post.tags.length - 2 }}
              </span>
            </div>
            
            <!-- Read more link -->
            <div class="flex items-center text-blue-400 text-sm font-medium group-hover:text-blue-300 transition-colors mt-auto">
              <span>Read more</span>
              <Icon name="mdi:arrow-right" class="w-4 h-4 ml-2 transform group-hover:translate-x-1 transition-transform" />
            </div>
          </NuxtLink>
        </article>
      </div>
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="text-gray-400 max-w-md mx-auto mb-8">
          I'm working on some great content about web development and software engineering. Check back soon!
        </p>
      </div>
